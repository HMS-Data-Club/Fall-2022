...
We can get fancy with calculating fold change:
```{r fancy}
#The fancy way
calculateFoldChange <- function(phos_plex, plex_names) {
  plex_data <- gather(phos_plex[,c("Uniprot","Isoform",plex_names)], "Sample","Abundance",-c("Uniprot","Isoform"))
  plex_data$Abundance <- as.numeric(plex_data$Abundance)
  plex_data$Condition <- apply(str_split_fixed(plex_data$Sample, "_", 3)[, c(1, 2)], 1, paste, collapse ="_")
  plex_data$Time <- str_split_fixed(plex_data$Sample, "_", 3)[, 2]
  plex_data$CLine <- str_split_fixed(plex_data$Sample, "_", 3)[, 1]
  summ <- plex_data %>%
    group_by(Uniprot, Isoform, Condition) %>%
    summarize(abundance_mean = mean(Abundance))
  summ$Time <- str_split_fixed(summ$Condition, "_", 2)[, 2]
  summ$CLine <- str_split_fixed(summ$Condition, "_", 2)[, 1]

  folds <- summ %>%
    group_by(Uniprot, Isoform,CLine) %>%
    summarize(fold_change = log2(abundance_mean[2]/abundance_mean[1])) %>%
    spread(., CLine, fold_change)
}
```

We're not going to get too far into what this function does.

However, since for now we're only interested in WT data, let's do things more simply. We'll calculate fold changes 'manually'. 
