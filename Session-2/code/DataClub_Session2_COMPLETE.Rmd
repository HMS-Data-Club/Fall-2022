...
```{r regular facet wrap, warning = FALSE, fig.width = 11, fig.height = 11}
...
For fun: the geofacet package works similarly as `facet_wrap()`, except it arranges panels following a grid that mimics the original topology. For more information on geofacet, see [here](https://cran.r-project.org/web/packages/geofacet/vignettes/geofacet.html).

```{r geofacet, warning = FALSE, fig.width = 14, fig.height = 8}
# load library
library(geofacet)
...
```{r college mask visual, warning = FALSE, fig.width = 6, fig.height = 4}
# import datasets
...
```{r college mask interactive plot, warning = FALSE, message = FALSE, fig.width = 6, fig.height = 4}
# load libraries
library(plotly)
library(htmlwidgets)
...
```{r static map, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 6}
# load libraries
library(maps)
...
```{r animated, echo=TRUE, results = FALSE}
# load libraries
library(gganimate)
library(gifski)
